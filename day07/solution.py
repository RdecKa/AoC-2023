...
class Hand:
    def __init__(self, hand_str, bid) -> None:
        self.hand_str = hand_str
        self.bid = bid
        self.type = self.get_hand_type(hand_str)

    def __repr__(self):
        return f"({self.type.name}, {self.hand_str})"

    def get_hand_type(self, hand_str):
        d = defaultdict(int)
...
def compare_hands(h1: Hand, h2: Hand):
    """Returns negative value if h1 is weaker than h2, and positive value if h1 is stronger than h2"""
    if h1.type != h2.type:
...
class Day7(Puzzle):
    ...
    def star1(self):
        hands = list(self.filereader.lines(parse_line))
